...
class DashboardFavWorkOrdersPage extends StatefulWidget {
  /// Receiving workorders from [DASHBOARD PAGE]
  final List<dynamic> workOrders;
  final String title;
  final Color? color;

  const DashboardFavWorkOrdersPage({
    super.key,
  ...
  @override
  State<DashboardFavWorkOrdersPage> createState() =>
      _DashboardFavWorkOrdersPageState();
}

/////////////////////////// STATE \\\\\\\\\\\\\\\\\\\\\\\\\\

class _DashboardFavWorkOrdersPageState extends State<DashboardFavWorkOrdersPage> {
                                            ...
                                            children: [
                                              ElevatedButton(
                                                onPressed: () async {
                                                  ...
                                                  try {
                                                    ...
                                                    await WorkOrderStatusUpdateApi.saveWorkOrder(
                                                      workOrder['id']!,
                                                      isSelectedData!, // Pass the toggled state
                                                      workOrder['assetId']!,
                                                    );

                                                    // Optionally, update state or show success feedback here
                                                  } catch (error) {
                                                    // Handle API errors gracefully
                                                    print('Error saving work order status: $error');
                                                    // Optionally, reset the state or show error feedback
                                                    // setState(() {
                                                    //   selectedItem[index] = !isSelectedData!; // Revert to the previous state
                                                    //   isSelectedData = selectedItem[index];
                                                    // });
                                                  }
                                                },
                                                style: ElevatedButton.styleFrom(
                                                  ...
                                                  padding: const EdgeInsets.all(12), // Adjust button padding
                                                ),
                                                child: Icon(
